...
/** ユーザーの既存 ~/.codex を使ってローカルで app-server を1つ動かす。 */
export class CodexAppServer extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    this.initializingPromise = (async () => {
      // 既知のクライアント識別子を使うことで、バックエンドがフルのエージェント
      // ツールセット(shell / file_change / apply_patch)をロードする。
      await this.send("initialize", {
        clientInfo: {
          name: "codex_cli",
          title: "MovieEditor (codex-cli compatible)",
          version: "0.1.0",
        },
